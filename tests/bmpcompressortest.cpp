...
void BmpCompressorTest::testEncodingDecodingRow()
{
    ...
    const std::string decoded1 = stringify({0xff,0xff,0xff,0xff,0x00,0x00,0x00,0x00,0x01,0x02,0x04,0x08}); // num % 4 = 0 case
    const std::string encoded1 = stringify({0x00,0x02,0x03,0x01,0x02,0x04,0x08});

    const std::string decoded2 = stringify({0xff,0xff,0xff,0xff,0x00,0x00,0x00,0x00,0x01,0x02,0x04,0x08,0x09,0x11}); // num % 4 = 2 case
    const std::string encoded2 = stringify({0x00,0x02,0x03,0x01,0x02,0x04,0x08,0x03,0x09,0x11});
